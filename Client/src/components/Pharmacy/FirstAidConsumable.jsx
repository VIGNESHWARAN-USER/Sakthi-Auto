...
const FirstAidConsumables = () => {
    ...
    const [firstAidConsumables, setFirstAidConsumables] = useState([]);
    ...
    const fetchFirstAidConsumables = async () => {
        try {
            setLoading(true);
            const params = {};
            ...
            const response = await axios.get("http://localhost:8000/get_first_aid_consumable/", { params });
            setFirstAidConsumables(response.data.first_aid_consumables || response.data);
            setLoading(false);
        } catch (error) {
            console.error("Error fetching first aid consumables:", error);
            setLoading(false);
        }
    };

    useEffect(() => {
        fetchFirstAidConsumables();
    }, []);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.post("http://localhost:8000/add_first_aid_consumable/", formData);
    ...
    const handleDownloadExcel = () => {
        const dataToExport = firstAidConsumables.map((item) => ({
        ...
        XLSX.writeFile(workbook, `FirstAid_Consumables_${Date.now()}.xlsx`);
    };
        ...
        <div className="h-screen w-full flex bg-gradient-to-br from-blue-300 to-blue-400">
            ...
            <div className="w-4/5 p-8 overflow-y-auto">
                ...
                <div className="mb-8 flex justify-between items-center">
                    <h1 className="text-4xl font-bold text-gray-800">First Aid Items</h1>
                    {!showForm && (
                        <button
                ...
                {!showForm ? (
                    ...
                    >
                        <div className="mb-6 flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-4 items-end">
                            ...
                            <button onClick={fetchFirstAidConsumables} className="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 flex items-center gap-1 text-sm transition">
                                <FaFilter size={12} /> Get
                            </button>
                            <button onClick={() => { setFromDate(""); setToDate(""); fetchFirstAidConsumables(); }} className="bg-gray-500 text-white px-4 py-2 rounded-md hover:bg-gray-600 flex items-center gap-1 text-sm transition">
...
export default FirstAidConsumables;
